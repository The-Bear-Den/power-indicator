 ...
 *
 * @param site_id Pointer to a buffer to store the site ID. Must be at least @ref SITE_ID_MAX_LEN
 * bytes long
 *
...
static bool fetch_site_id(char *site_id)
{
    ...
    snprintf(request, sizeof(request), AMBER_GET_REQUEST_FORMAT_STRING, WEB_URL,
             CONFIG_AMBER_API_KEY);
